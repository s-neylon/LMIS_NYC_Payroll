...
Clean title names to get rid of * and ?
...
nyc_agency <- nyc_data %>% group_by(agency)

nyc_titles <- nyc_agency %>% count(title, sort = TRUE)
...
nyc_titles_payu <- left_join(nyc_titles, nyc_wages, by = title)
...
write_csv(nyc_titles, "nyc_titles_BYagency.csv")
...
nyc_wages_byAGENCY <- nyc_group %>% summarise(
  tot_emp = n(),
  avg_pay = mean(reg_grosspaid),
  med_pay = median(reg_grosspaid),
  avg_pay = mean(reg_grosspaid),
...
write_csv(nyc_wages_byAGENCY, "nyc_wages_BYagency.csv")
