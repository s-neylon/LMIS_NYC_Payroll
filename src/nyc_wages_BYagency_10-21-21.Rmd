...
nyc_wages <- nyc_group %>% summarise(
  ...
  avg_FTE = mean(FTE_pay),
  med_FTE = median(FTE_pay),
  entry = mean(FTE_pay[EntryTERT == TRUE]),
  exper = mean(FTE_pay[ExperTERT == TRUE]),
  avg_FTE21 = mean(FTE_pay21),
  med_FTE21 = median(FTE_pay21),
...
write_csv(nyc_wages, "nyc_wages_byAgency_FY20adj21_10-21-2021.csv")
